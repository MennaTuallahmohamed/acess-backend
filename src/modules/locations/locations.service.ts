...
export class LocationsService {
  ...
  private async safeFindMany(model: any, args?: any) {
    if (!model) return [];
  ...
  async findAll() {
    ...
    return this.safeFindMany(locationModel, {
      orderBy: {
        id: 'asc',
      },
    });
  ...
  async getLocationsScanSummary() {
    ...
    const locations = await this.safeFindMany(locationModel, {
      orderBy: {
        id: 'asc',
      },
    });

    const devices = await this.safeFindMany(deviceModel, {
      orderBy: {
        id: 'asc',
      },
    });

    const inspections = await this.safeFindMany(inspectionModel, {
      orderBy: {
        createdAt: 'desc',
      },
    });

    const technicians = await this.safeFindMany(technicianModel);
