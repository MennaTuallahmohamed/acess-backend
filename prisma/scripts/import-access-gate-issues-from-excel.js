...
function readSheetRows(workbook, preferredSheetName) {
  const sheetName =
    ...
    workbook.SheetNames.find((name) => name.includes(preferredSheetName.trim())) ||
    preferredSheetName;
  ...
  if (!sheet) {
    throw new Error(
      `Sheet not found: ${preferredSheetName}. Available sheets: ${workbook.SheetNames.join(', ')}`,
    );
  }
...
async function insertReturningId(tableName, data) {
  ...
  const valueSql = entries
    .map(([key], index) => {
      const enumType = enumCasts[tableName]?.[key];

      if (enumType) {
        return `$${index + 1}::"${enumType}"`;
      }
...
async function getOrCreateCategory(name, description) {
  ...
  if (existing.length) {
    ...
    if (columns.has('status')) {
      updates.push(`"status" = 'ACTIVE'::"IssueStatus"`);
    }
